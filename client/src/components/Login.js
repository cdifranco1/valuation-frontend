  ...
  return (
    <div className="bg-gray-300 h-screen">
      <form onSubmit={handleSubmit} className="border border-blue-800 rounded-lg overflow-hidden w-1/3 mx-auto login-position">
        <h1 className="text-3xl font-bold text-white p-4 bg-blue-800 tracking-wide">{isRegistration ? "Register" : "Login"}</h1>
        ...
        <div>
          <button onClick={changeForm} id="registration">Register</button>
          <button onClick={changeForm} id="login">Login</button>
        </div>
      </form>
    </div>
  )
